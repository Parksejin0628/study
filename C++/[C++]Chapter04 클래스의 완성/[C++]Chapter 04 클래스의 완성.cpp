...
1. 정보 은닉은 멤버 변수를 private로 선언한 뒤 해당 변수에 접근하는 별도의 함수를 선언하여 안전하고 제한된 형태로 멤버 변수에 접근하도록 유도하는 방식이다.
2. 엑세스 함수는 클래스 외부에서의 멤버변수 접근을 목적으로 정의된 함수들을 의미한다. 당장 호출하지 않아도 정의를 해두는 것이 좋다.
3. 함수 뒤에 const가 붙는 경우 멤버 변수의 변경을 허용하지 않는다. 또한, const 함수내에서는 const함수만 호출이 가능하다.
  > 값의 변화가 없는 경우 const를 붙여 코드의 안정성을 높이고 함수의 의도를 직관적으로 보이게 하자.

<4..2 캡슐화(Encapsulation)>
1. 캡슐화는 하나의 목적을 이루기 위한 기능들을 한 곳에 모아 코드를 간결하게 하고 객체들의 상호작용을 쉽게 파악할 수 있게 하는 것이다.
2. 캡슐화를 함으로써 프로그램의 복잡도를 낮추고 객체의 활용을 쉽게 하며 진행순서가 중요한 경우 구조가 무너짐을 방지할 수 있다.
3. 캡슐화의 범위를 결정하는 것은 정답이 없고 프로그램의 성격과 특성에 따라 달라지기에 많은 경험이 필요하다.

<4-3. 생성자(Constructor)와 소멸자(Destructor)>
1. 생성자 : 객체 생성시 딱 한 번 호출되는 함수
	- 형태 : 클래스 이름과 함수 이름이 같으며 반환형을 선언하지 않는다.
	- 용도 : 객체의 생성과 동시에 멤버 초기화
	- 특징
		> 생성자가 매개변수를 가지고 있으면 객체를 정의할 때 생성자의 인자를 넣어야 한다.
		> 오버로딩, 디폴트 값 설정이 가능하다.
		> 생성자가 없어도 자동으로 디폴트 생성자가 삽입된다.
	- 멤버 이니셜 라이저 : 생성자에서 멤버 선언과 동시에 초기화 할 수 있는 기능
		> 형태 : 생성자 함수명 뒤에 : 대상(초기화값), 대상(초기화값), ...
			ex) SimpleClass(int num) : val(30), ...
		> 멤버를 초기화 할 때 기본적으로 사용해야 한다.
2. 소멸자 : 객체 소멸 시 딱 한 번 호출되는 함수
	- 형태 : 클래스 이름 앞에 ~가 붙고 반환형과 매개변수 모두 없는 형태
		ex) ~SimpleClass()
	- 용도 : 주로 할당한 메모리를 반환할 때 사용한다.
	- 특징
		> 소멸자가 없어도 자동으로 디폴트 소멸자가 삽입된다.
		> 함수 오버로딩, 디폴트 값은 불가능하다.
3. 객체의 생성 과정
	1) 메모리 공간 할당
	2) 이니셜라이저를 이용한 멤버의 초기화
	3) 생성자 몸체 부분 실행
		> 이 과정이 필요하므로 디폴트 생성자라도 넣는다.

<4-4. 클래스와 배열 그리고 this 포인터>
1. 객체 배열의 선언 방식은 크게 두 가지 이다.
	- 객체 배열 선언
		> 형태 : ClassName arr[10];
	- new 연산자를 통한 동적 할당
		> 형태 : ClassName *ptrArr = new ClassName[10];
		> 후에 delete로 반환을 해야 한다.
2. 객체 배열 선언 시 주의할 점
	- 생성자를 호출하지만 인자를 전달할 수 없으므로 인자를 받지 않는 생성자가 존재해야 배열을 선언할 수 있다.
	- 배열로 선언한 객체를 초기화하고 싶다면 하나씩 별도로 초기화 해야 한다.
	- 배열이 소멸할 때 각 객체의 소멸자가 동시에 호출된다.
3. this 포인터 : 객체 자신의 주소값을 의미한다. 즉, 자기 자신을 가리킨다.
...
<4-4. 클래스와 배열 그리고 this 포인터> 
1. 객체 배열
 - 객체 배열을 선언하는 방식은 크게 두 가지이다.
  > ClassName arr[10];	: 변수의 배열선언처럼 객체 배열 선언
  > ClassName *ptrArr = new ClassName[10];	: new 연산자를 사용해 동적으로 할당. 이 때 주의할 점은 하나의 클래스 포인터 자체는 배열이 아니라는 것이다.
 - 객체 배열 선언 역시 생성자를 호출하지만 인자를 전달할 수 없다. 따라서, 배열로 선언하기 위해서는 인자를 받지 않는 생성자가 존재해야 한다.
 - 만약, 배열로 선언한 객체를 초기화하고 싶으면 하나하나 별도로 초기화를 해야한다.
 - 배열이기 때문에 배열 안에 모든 객체의 소멸자도 모든 객체가 동시에 호출된다. 
2. 객체 포인터 배열
 - 객체로 이루어진 배열이 아닌 객체의 주소를 저장하는 포인터 변수로 이루어진 배열이다.
 - 객체 배열과 다르게 포인터 변수에 인덱스[]를 붙인다.
  > ClassName *ptrArr[10]; (객체 배열에서는 *ptrArr에 []를 붙이지 않았다.)
 - 객체를 동적으로 할당하는 것과 객체 포인터 배열은 헷갈릴 수 있으니 조심하자.
  > 포인터 변수에 배열이 있냐 아니냐로 구별이 가능할 듯 하다.
3. this 포인터
 - this 포인터는 객체 자신의 주소값을 의미한다. 즉, 자기 자신을 가리키기 위해 사용한다.
 - this 포인터를 사용함으로써 매개변수와 멤버변수의 이름이 같아도 구별할 수 있다. 즉, 이름을 같게 할 수 있다.
 	ex) ThisFunc(int num) { this->num = num; (this를 통해 매개변수와 멤버변수를 구별) }
 - 또한, 자기 자신을 참조하는 Self-Reference에서 활용이 된다. 
*/
