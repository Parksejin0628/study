...
2. 클래스를 기반으로 선언한 포인터 변수는 해당 클래스의 유도 클래스 객체도 가리킬 수 있다.
	ex) Person를 상속하는 Student, Student를 상속하는 HighSchoolStudent가 있다고 하자
	Person *ptr로 선언한 경우 ptr은 Student, HighSchoolStudent 객체 모두를 가리킬 수 있다.
	> 이를 통해 유도 클래스의 포인터를 따로 선언할 필요가 없어 유연성과 확장성을 확보한다.
3. 함수 오버라이딩 : 기초클래스에서 상속받는 함수를 유도 클래스에서 동일한 이름과 형태로 재정의 하는 것이다.
	> 유도클래스에서 해당 함수를 호출할 경우 기초클래스의 함수가 아닌 유도클래스에서 재정의된 함수가 호출되게 된다.

<8.2. 가상함수>
1. 클래스형 포인터 변수는 유도클래스도 가리킬 수 있으나, 포인터 변수 연산의 근거는 선언한 자료형을 기준으로 한다.
	- 멤버를 호출할 때 유도클래스를 가리키는 포인터여도 포인터 변수를 선언한 기초 클래스의 멤버만 가져올 수 있다.
	- 대입 연산을 진행할 때 유도클래스를 카리키는 포인터여도 유도클래스로 선언한 포인터에 대입 연산은 불가능하다.
		ex) Base *bptr = new Derived(); / Derived *dptr = bptr (x)
	- 오버라이딩된 함수가 존재해도 처음 선언한 클래스의 함수를 가져온다. 즉, 유도클래스의 함수가 아닌 기초클래스의 함수를 가져온다.
2. 가상함수(virtual) : 클래스형 포인터 변수가 오버라이딩된 함수를 호출할 때 자료형이 아닌 가리키는 대상의 함수를 가져오게 하는 개념이다.
	- 함수 앞에 virtual을 붙이면 해당 함수를 오버라이딩한 함수 모두 가상함수가 된다.
	- 확장성과 유연성을 확보하기 위해 등장하였다.
	- 가상함수 끝에 = 0을 붙이면 순수 가상함수가 되며 해당 클래스는 기초 클래스로서의 의미만 가지는 추상 클래스가 된다.
3. 다형성(Polymorphism) : 모습은 같은데 형태는 다른, 즉 같은 문장이어도 다른 결과를 도출해내는 가상함수의 특성을 의미한다.

<8.3. 가상 소멸자와 참조자의 참조 가능성>
1. 클래스형 포인터 변수는 자료형이 따라가므로 소멸자도 의도한 대로 실행시키기 위해 앞에 virtual을 붙인다. (가상 소멸자)
2. 참조자도 포인터와 같이 접근할 수 있으며 선언한 자료형을 따른다, virtual을 따른다 등 포인터와 성질이 동일하다.
*/
