...
<7.1. 상속에 들어가기에 앞서>
1. 상속은 문법적 요소보다 상속이 가져다 주는 이점, 적용해야 할 때를 아는 것이 중요하다.
2. 좋은 소프트웨어가 되기 위해 중요하게 생각해야 하는 요소는 다음과 같다.
	- 요구사항 변경에 대응하는 프로그램의 유연성
	- 기능의 추가에 따른 프로그램의 확장성
3. 즉, 좋은 프로그램은 추가 요구에 쉽게 대응할 수 있어야 하며 이를 위해 필요하게 된 것이 상속이다.

<7.2. 상속의 문법적인 이해>
1. 상속은 다른 클래스의 멤버를 물려 받아 멤버 변수와 멤버 함수를 포함하게 되는 것이다.
	- 정의 방법은 'class 클래스이름 : public(protected/private) 상속을주는클래스이름'처럼 한다.
	- 상속을 해주는 클래스는 기초 클래스, 받는 클래스는 유도클래스라고한다.
2. 기초클래스의 멤버를 초기화 할 때는 유도클래스 생성자의 이니셜라이저에서 기초클래스의 생성자를 호출하는 방식으로 한다.
	ex) (생성자)SimpleClass(int n1, int n2) : baseClass(n1), num(n2) < 여기서 baseClass가 기초클래스 이름 즉, 기초클래스의 생성자
	- 클래스의 멤버는 해당 클래스의 생성자를 통해서 초기화해야 한다는 기본 원칙이 적용된 것이며 이는 소멸자도 마찬가지이다.
3. private선언된 멤버는 유도클래스에서도 접근할 수 없다.
	- 즉, 정보의 은닉은 상속관계에서도 이루어진다.

<7.3. protected 선언과 세 가지 형태의 상속>
1. protected : 유도클래스에서만 접근이 가능하게 하는 지시자로 private, public 중간이라고 보면 된다.
2. 세 가지 형태의 상속
	- public 상속 : private를 제외한 모두를 상속한다. 가장 일반적이다.
	- protected 상속 : public 멤버변수를 protected 멤버 변수로 상속한다. 나머지는 public과 같다.
	- private 상속 : public, protected 멤버변수를 private로 상속받는다.
		> private 상속이 이루어진 클래스를 상속하면 이미 모두 private이기에 상속이 의미가 없다.

<7.4. 상속을 위한 조건>
1. 상속의 기본 조건은 IS-A관계가 되어야 한다.
	ex) 스마트폰은 전화기이다. 여기서 스마트폰이 유도클래스, 전화기가 기초클래스이다.
2. 이러한 관계는 모든 단계의 관계에서 이루어져야 한다.
	ex) 갤럭시폰은 스마트폰이다 / 스마트폰은 전화기이다 / 갤럭시폰은 전화기이다. (옳은 상속)
3. HAS - A 관계는 상속이 가능하긴 하지만 유연성과 확장성이 떨어지기에 하지 않는 것이 좋다.
	ex) 경찰은 총을 가진다. 여기서 경찰이 유도클래스, 총이 기초클래스가 될 수 있다.
		> 하지만, 총을 들지 않는 경찰을 표현하려면 큰 수정이 필요하기에 유연성이 떨어진다.
*/
