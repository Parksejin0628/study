...
1. 대입 연산자는 복사 생성자와 매우 유사하다.
	- 디폴트 대입 연산자의 존재 (디폴트 복사 생성자)
	- 디폴트 대입 연산자는 맴버 대 맴버 (얕은 복사)
	- 깊은 복사가 필요하면 따로 정의해야 한다.
	- 대입연산자는 복사 생성자와 다르게 이미 초기화가 끝난 두 객체를 대상으로 한다는 것이 큰 차이이다.
2. 복사 생성자와 마찬가지로 참조형 변수가 있을 때 얕은 복사는 문제를 일으킨다.
	- 같은 대상을 가리키는 문제가 발생한다.
	- 심지어, 대입 연산자는 이미 초기화된 객체를 대상으로 하므로 원래 가리키던 주소값을 잃게 되어 소멸이 불가능해진다.
		> 이는 메모리 누수로 이어진다.
	- 따라서, 깊은 복사를 따로 정의해야 하는 경우가 생긴다.
3. 대입 연산자는 복사 생성자와 다르게 유도 클래스에서 대입 연산자가 호출되어도 기초 클래스의 대입 연산자는 호출되지 않는다.
	- 기초 클래스의 멤버에 대한 대입을 유도 클래스 멤버 함수에 따로 명시해야 한다는 의미이다.

<11.2. 배열의 인덱스 연산자 오버로딩>
1. []연산자도 연산자 오버로딩이 된다. 이를 통해 경계검사를 실시하지 않는 []연산자의 단점을 보완할 수 있다.
	- 형태 : operator[]()
2. 연산자 오버로딩 함수 또한 const함수에 대응할 수 있어야 하므로 const 오버로딩을 해두는 것이 좋다.
3. 객체의 주소값들을 저장하는 주소값 저장 배열 기반 클래스는 복사 생성자, 대입 연산자의 얕은 복사를 신경 쓸 필요가 없어 용이하다.
...
1. new, delete 연산자 오버로딩
	- new
		...
		> 기본 형태 안에서 오버로딩이 이루어져야 하며 특히 정의부분은 바뀌어서는 안된다.
	- delete
		> 기본 형태
		...
		> 기본 형태 안에서 오버로딩이 이루어져야 하며 특히 정의부분은 바뀌어선 안된다.
		> 배열형인 경우 정의부분 delete뒤에[]만 추가하면 된다.
2. *, -> 오버로딩
	- * 오버로딩 : 자료형& operator*() { ...; return *this; }
	- -> 오버로딩 : 자료형* operator->() { ...; return this; }
3. (), 형변환 오버로딩
	- () 오버로딩 : 반환형 operator()(매개변수)
		> 객체를 함수처럼 사용 가능
	- 형변환 오버로딩 : operator 변환형 () { ... };
