...
1. 연산자 오버로딩 : 객체에서도 연산자를 통한 함수호출이 가능하게 하는 것
	> 방법 : '리턴형 operator연산자(연산대상)' 형태로 함수를 선언한다.
		ex) 멤버함수의 경우 : Point oprator+(const Point &ref)
			 전역함수의 경우 : Point oprator+(const Point &pos1, const Point &pos2)
		- 멤버함수 혹은 전역함수로 가능하나 전역함수의 경우 객체 접근을 위해 friend 선언을 해두어야 한다.
2. 컴파일러는 객체의 연산을 볼 때 연산자를 .operator연산자(연산대상) 형식으로 치환해서 해석한다.
	ex) pos1 + pos2는 pos1.oprator+(pos2)로 바뀌는 것이다.
3. 연산자 오버로딩의 주의사항
 1) const로 선언하여 피연산자 변경 방지
 2) 연산자의 우선순위와 결합성은 바뀌지 않음 ('+'보다'*'가 우선)
 3) 매개변수에 디폴트 값 설정 불가능
 4) 기본 자료형의 연산자는 간섭 불가능

<10.2. 단항 연산자의 오버로딩>
1. 단항 연산자는 피연산자가 하나이기에 매개변수를 하나씩 줄이면 된다.
	ex) 멤버함수 : Point operator++()
	ex) 전역함수 : Point operator++(Point &ref)
2. 전위증가/감소 형태의 단항 연산자
	- 증가하고자 하는 멤버를 증가시킨 후 return *this를 한다. 자기 자신의 참조값을 반환한다는 의미이다.
	- Point.cpp의 Point Point::oprator++()함수 참고
3. 후위증가/감소 형태의 단항 연산자
	- 전위연산자와 구별을 위해 매개변수 int를 추가한다. 또한, 리턴형은 const 객체명& 으로 해둔다.
	- 먼저 자기자신의 객체를 저장한 후 멤버를 증가시키고 저장해둔 객체를 반환한다.
		> 후위 연산은 다른 연산이 끝난 후 자기 자신을 증가시키기에 변화하기 전 객체를 리턴한다.
	- Point.cpp에 있는 const Point& Point::operator(int)함수 참고

<10.3. 교환법칙 문제의 해결>
1. 연산자 오버로딩을 이용하면 자료형이 다르더라도 연산을 진행할 수 있다.
2. 교환법칙 문제 해결을 위해서는 객체가 아닌 자료형도 연산자 오버로딩을 시켜야 한다.
	- 전역함수 형태로 매개변수를 여러 개 받아 해결한다.
	- Point.cpp의 Point oprator+(int numm, const Point &pos)를 참고
*/
