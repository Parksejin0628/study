...
1. const로 객체를 선언하면 상수화되며 선언 이후로 데이터 변경을 허용하지 않는다는 의미이다.
2. 상수화된 객체는 const 선언이 된 멤버 함수만 호출이 가능하다.
3. const 선언 유무만으로 함수 오버로딩이 되며 const객체는 const함수를, 일반 객체는 일반 함수를 호출하게 된다.

<6-2. 클래스와 함수에 대한 friend 선언>
1. friend은 friend선언을 한 클래스의 private 멤버를 friend로 선언한 대상(객체 혹은 함수)가 접근할 수 있도록 허용한다는 의미이다.
2. friend선언은 정보은닉을 무너뜨리기에 가급적 사용하지 않는 것이 좋다.
3. 함수명만 미리 선언하듯이 클래스도 이름만 미리 선언하면 다른 곳에서도 해당 클래스가 있다는 사실을 인지시킬 수 있다.

<6-3. C++에서의 static>
1. 클래스의 멤버에 static선언이 가능하다. static선언시 특징은 다음과 같다.
	- 객체의 생성과 소멸에 영향을 받지 않고 처음부터 존재해있고 초기화되지 않는다.
	- 클래스 안에서 있지만 객체 안에 존재하는 것이 아닌 객체에게 접근할 수 있는 권한을 준 것이다.
		> 객체에게 허용된 전역 변수/함수라고 생각하면 편하다.
	- 객체안에 존재하는 것이 아니기에 클래스 이름으로 접근이 가능하다.
2. static 멤버변수는 다음의 특징을 가진다.
	- 생성자로 초기화하면 안된다. 객체가 생성될 때마다 초기화되므로 static의 의미가 퇴색된다.
	- 클래스 외부에서 초기화 하는 것이 일반적이다.
		ex) int SimpleClass::staticVar = 2;
	- public인 경우 클래스 이름으로 접근이 가능하다.
		ex) SimpleClass::staticVar
	- const 선언이 된 경우 클래스 내에서 일반 변수처럼 초기화가 가능하다.
3. static 멤버함수는 다음의 특징을 가진다.
	- 객체의 멤버가 아니기 때문에 객체의 멤버 변수나 함수에 접근할 수 없다.
	- 따라서, static 멤버변수와 멤버함수만 호출이 가능하다.
	- public 선언시 클래스 이름으로 접근이 가능하다.
