...
1. 멤버함수는 객체 안에 존재하는 것이 아닌 별도의 메모리 주소에 존재한다.
	> 객체는 멤버함수가 존재하는 메모리 주소만 저장하는 방식이다.
2. 객체에 가상함수가 존재할 경우 유도클래스에서는 오버라이딩된 가상함수의 주소값만 저장하고 기초클래스의 주소값은 저장하지 않는다.
	> 이러한 원리로 마지막에 오버라이딩한 유도 클래스의 멤버함수를 불러온다.
3. 이렇게 한 번 주소를 거쳐야 하기 때문에 C++가 C보다는 느리다.
	> 그러나 이렇게 하는 것이 단점에 비해 장점이 크기에 유용하게 활용된다.

<9.2. 다중상속(Multiple Inheritance)에 대한 이해>
1. 다중상속은 둘 이상의 클래스를 동시에 상속하는 것이다.
	ex) class AAA : public BBB, protected CCC { ... };
2. 득보다 실이 되는 경우가 많기에 사용을 자제해야 한다.
3. 다중상속의 문제점과 해결점은 다음과 같다.
	1) 기초 클래스들에서 동일한 이름의 멤버가 존재하는 경우 문제 발생
		- 어느 기초클래스에서 멤버를 가져올 것인지 정의하여 해결
			ex) CCC::SimpleFunc();
	2) 기초 클래스들이 더 상위의 같은 기초클래스 상속할 경우 그 상위 기초클래스의 멤버를 두 번 이상 상속받음
		- 가상 상속을 사용해 해결
*/
...
2. 가상함수의 동작원리 (virtual)
 ...
 2) 가상함수 테이블은 실제 호출되어야 하는 함수의 위치정보(포인터값, 위 표에서는 value)를 담고 있는 테이블이다.
	> 가상함수 테이블을 가리키는 주소값이 객체 내에 저장되지만, 개발자가 참조할 수 없는 값이다.
